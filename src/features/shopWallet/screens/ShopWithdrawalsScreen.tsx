import { useShopWalletSummary } from "@/features/shopWallet/hooks/useShopWalletSummary";
...
export function ShopWithdrawalsScreen({ navigation }: any) {
  ...
  const { data, isLoading, isError, refetch } = useShopWithdrawals(currentPage, 10);
  const { data: walletData } = useShopWalletSummary();
  const createWithdrawalMutation = useCreateShopWithdrawal();
  const cancelWithdrawalMutation = useCancelShopWithdrawal();

  const withdrawals = data?.result || [];
  const walletBalance = walletData?.result?.availableBalance || 0;
          ...
          onPress: () => {
            cancelWithdrawalMutation.mutate(withdrawalId, {
              onSuccess: () => {
                Alert.alert(
                  "Thành công",
                  "Yêu cầu rút tiền đã được hủy. Số tiền đã được hoàn lại vào ví Shop.",
                  [
                    {
                      text: "OK",
                      onPress: () => refetch(),
        ...
        {withdrawals.length === 0 ? (
          <View className="bg-white dark:bg-dark-card rounded-3xl p-8 items-center shadow-sm mt-2">
            ...
            <Text className="text-light-textSecondary dark:text-dark-textSecondary text-sm text-center">
              Các yêu cầu rút tiền của Shop sẽ hiển thị ở đây
            </Text>
          </View>
        ) : (
          <View className="pb-6">
